...
module app {
    ...
    function routehelper($location, $rootScope, $stateParams, $state, logger, $injector) {
        ...
        $rootScope.$on('$locationChangeSuccess', function (evt) {
            ...
            if (path != normalized) { return; } /* ignore non-normalized url, since normalized will come right after. (avoid double popup) */
        ...
        $rootScope.$on('$stateChangeError', function (event, toState, toParams, fromState, fromParams, error) {
            //any errors in resolve functions: javascript errors, non-existent services, 401, etc...
